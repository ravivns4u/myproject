...
import { addComments } from "../../../firebase/db/comments/addComments";
...
const countHandler: NextApiHandler = async (req, res) => {
  ...
  if (!Response) {
    res.status(400).json(eR({ msg: "Failed to Add Comment. Kindly Retry." }));
    return;
  } else res.status(201).json(gR({ msg: "Commented Successfully" }));
};
