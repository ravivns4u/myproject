...
import { deleteImagePortfolio } from '../../../firebase/db/portfolios/portfolioUpdate';
import {
  errorResponse as eR,
  genericResponse as gR,
} from '../../../lib/backend/responseSynthesizer';
import type { ImagePortfolioData } from '../../../redux/interfaces/backend/apis/ImagePortfolio';
interface userDetails {
  firebaseToken: string;
  portfolioDetails: ImagePortfolioData;
}

const countHandler: NextApiHandler = async (request, response) => {
  const { firebaseToken, portfolioDetails } = request.body as userDetails;
  try {
    ...
    const result = (await deleteImagePortfolio(portfolioDetails)) as {
      error: boolean;
    };
