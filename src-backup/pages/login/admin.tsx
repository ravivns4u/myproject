...
import LoginForm from "../../components/Common/MUIComponents/LoginForm/Vibe/IndividualAdmin";
import { individualLoginImagePaths } from "../../components/constants/loginUiScreensAdmin";
  ...
  React.useEffect(() => {
    if (fbUser && isLoggedIn && merchantSlug && user) {
      if (!adminApproved || !emailVerified) {
        ...
        router.push(`/admin/${merchantSlug}/portfolio/images`);
      } else {
        router.push(`/admin/${merchantSlug}/subscription`);
      }
    }
    if (!fbUser) {
