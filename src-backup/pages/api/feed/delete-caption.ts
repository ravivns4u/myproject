...
import {deleteCaption} from '../../../firebase/db/feeds/deleteCaption';
...
const countHandler: NextApiHandler = async (req, res) => {
  ...
  const Response = await deleteCaption(payload);
  if (!Response) {
    res.status(400).json(eR({ msg: "Failed to Delete Caption. Kindly Retry" }));
    return;
  } else res.status(201).json(gR({ msg: "Caption Deleted Successfully" }));
};
