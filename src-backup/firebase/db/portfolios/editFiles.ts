...
export const editMerchant = async (req: EditPointer) => {
  ...
  switch (req.editOperationType) {
    case 'images': {
      return await updateImagePortfolio(data as ImagePortfolioData);
    }
    case 'services': {
...
const updateImagePortfolio = async (data: ImagePortfolioData) => {
  //Truncate more than 500 characters
  try {
    ...
    await portfolioCollection.doc(data.fp).set(data, { merge: true });
    return true;
  } catch (err) {
    console.log('Error happened ', err);
    return false;
