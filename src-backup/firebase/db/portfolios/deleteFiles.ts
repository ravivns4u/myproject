...
export const deleteMerchant = async (
  ...
  switch (data.deleteOperationType) {
    case 'images': {
      return await deleteImagePortfolio(data as DeletePointer);
    }
    case 'services': {
...
const deleteImagePortfolio = async (data: DeletePointer) => {
  const portfolioCollection = Server.db.collection(
    `${constDocumentRefs.users_portfolio_image}/${data.uid}`
  );
  try {
    await portfolioCollection.doc(data.collectionId).delete();
