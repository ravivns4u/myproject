...
import {
  ICategoryMap,
  InsertionType,
} from '../../../../../redux/interfaces/backend/apis/v2/common';
import { statesArray, citiesArray } from '../../../getCities';
...
export const deleteEvent = async (eventLocation: string) => {
  const eventDoc = Server.db.doc(eventLocation);
...
export const getEvents = async (
  ...
  const eventArray = (await Promise.all(eventsPromises)) as IAddEventFrontend[];
  return eventArray;
};
